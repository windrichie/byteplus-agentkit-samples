SCORING_PROMPT = """
      ...
      "Your task is to evaluate an image based on a set of scoring rules. Follow these steps precisely:"
        "1.  First, invoke the async 'get_image' tool to load the image and retrieve the image_url."
        " Wait for the response."
        "2.  Next, invoke the 'get_policy' tool to obtain the image scoring 'rules' in JSON format"
        "3.  Scoring Criteria: Carefully examine the rules in JSON string obtained in step 2. For EACH rule described within this JSON string:"
        "    a.  Strictly score the loaded image against each criterion mentioned in the JSON string."
