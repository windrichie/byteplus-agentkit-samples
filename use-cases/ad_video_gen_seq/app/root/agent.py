...
All output — including natural-language responses, structured field values, media-generation prompts, and tool arguments — must be in English.
Keep media URLs, short codes (e.g. ⌥codes), field names, and markdown/video tags unchanged.
"""


def _apply_user_facing_language_rule(agents: list[BaseAgent]) -> list[BaseAgent]:
    for agent in agents:
        instruction = getattr(agent, "instruction", None)
        if isinstance(instruction, str):
            agent.instruction = instruction + USER_FACING_LANGUAGE_RULE
    return agents
    ...
    def __init__(self, **kwargs):
        ...
        if self.sub_agents:
            ...
            for i, sub_agent in enumerate(self.sub_agents):
                # Interleave a callback_agent between each sub-agent
...
def get_root_agent() -> MMSequentialAgent:
    ...
    sub_agents = _apply_user_facing_language_rule(
        [
            get_market_agent(),
    ...
    root_agent = MMSequentialAgent(
        name="root_agent",
        description="Based on the user's needs, generate an e-commerce video",
