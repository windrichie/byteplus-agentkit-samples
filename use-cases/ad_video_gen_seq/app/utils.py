...
class UrlShortener:
    ...
    def code2url(self, short_id: str) -> Optional[str]:
        ...
        normalized_id = short_id.strip().strip("`").strip()
        resolved_url = self._short_to_long.get(normalized_id)
        if resolved_url:
            return resolved_url

        if normalized_id.startswith(self.PREFIX):
            code = normalized_id[len(self.PREFIX) :]
            if 0 < len(code) < 5 and code.isalnum():
                padded_id = f"{self.PREFIX}{code.rjust(5, '0')}"
                resolved_url = self._short_to_long.get(padded_id)
                if resolved_url:
                    logger.warning(
                        f"Resolved normalized short code {normalized_id} as {padded_id}."
                    )
                    return resolved_url
    ...
    def replace_in_text(self, text: str) -> str:
        """
        Given a long string, extract short IDs and seamlessly replace them back with the original URLs.
        """
        # Pattern matches ⌥<code>; tolerate model-dropped leading zeros.
        pattern = r"⌥([0-9a-zA-Z]{1,5})"
    ...
    def extract_ids_to_urls(self, text: str) -> list[str]:
        """
        Extract all short IDs from a string and convert them into a list of URLs.
        """
        # Pattern matches ⌥<code>; tolerate model-dropped leading zeros.
        pattern = r"⌥([0-9a-zA-Z]{1,5})"
...
def upload_file_to_tos(
    ...
    if not bucket_name:
        logger.info("Error: DATABASE_TOS_BUCKET is not configured.")
        return None
    ...
    if not (access_key and secret_key):
        ...
        try:
            ...
            logger.info(f"Unable to retrieve IAM credentials: {e}")

    if not access_key or not secret_key:
        logger.info(
            "Error: TOS credentials are not provided. Configure BYTEPLUS_ACCESS_KEY/BYTEPLUS_SECRET_KEY, VOLCENGINE_ACCESS_KEY/VOLCENGINE_SECRET_KEY, or an IAM role."
        )
        return None
    ...
    try:
        ...
        if not endpoint:
            if os.getenv("BYTEPLUS_ACCESS_KEY") or os.getenv("BYTEPLUS_REGION"):
        ...
        logger.info(f"TOS Region: {region}")
