...
def _resolve_supported_frame(value: str | None) -> str | None:
    if not isinstance(value, str) or not value.strip():
        return None

    resolved = url_shortener.code2url(value.strip())
    parsed = urllib.parse.urlparse(resolved)
    if parsed.scheme in {"http", "https", "data"}:
        return resolved

    logger.warning(f"Skip invalid video frame input: {value}")
    return None
...
async def video_generate(
        ...
        max_wait_seconds (int):
            ...
            Required per item:
                ...
                - prompt (str):
                    ...
                    Example:
                        "A kitten riding a skateboard through a park. --rs 720p --rt 16:9 --dur 5 --fps 24 --wm true --seed 11 --cf false"
    ...
    Model text commands (append after the prompt; unsupported keys are ignored by some models):
        ...
        --dur / --duration <seconds>      Clip length in seconds. Seedance supports **3–12 s**;
                                          Wan2.1 only supports 5 s. Default varies by model.
            ...
            params = [
                {
                    ...
                    "prompt": "A kitten riding a skateboard through a park. --rs 720p --rt 16:9 --dur 5 --fps 24 --wm false"
                },
                {
                    "video_name": "city_night.mp4",
                    "prompt": "Time-lapse of a city under neon lights at night. --rs 720p --rt 16:9 --dur 5 --fps 24 --seed 7"
                },
            ]
            ...
            params = [
                {
                    ...
                    "first_frame": "⌥abc12",
                    "last_frame": "⌥xyz34",
                    "prompt": "A brand logo transitioning from line art to full color. --rs 1080p --rt 1:1 --dur 6 --fps 24 --cf true"
                }
            ]
    """
    for item in params:
        if "prompt" in item:
            item["prompt"] = (
                f"(Very light action sounds are allowed, but no human voice, no background music, "
                f"no sound effects, no narration, no commentary is permitted.) {item['prompt']}"
            )
        if "first_frame" in item:
            item["first_frame"] = _resolve_supported_frame(item.get("first_frame"))
        if "last_frame" in item:
            item["last_frame"] = _resolve_supported_frame(item.get("last_frame"))
